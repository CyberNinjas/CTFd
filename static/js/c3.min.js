...
function chalSearch(needle, haystack,showAlert){
  needle = needle.toUpperCase();
  if(needle.length < 3)
    return;
  for (var i = 0; i < haystack.length; i++) {
    var item = haystack[i];
    if(item.name.toUpperCase().indexOf(needle) != -1 || item.description.toUpperCase().indexOf(needle) != -1){
      loadchalbyname(item.name);
      return;
    }
  }
  if(showAlert)
    alert("No challenges match your query.");
}
$("#search").keyup(function(event){
    if(event.keyCode == 13){
      chalSearch(this.value, challenges.game, true);
    } else {
      chalSearch(this.value, challenges.game, false);
    }  
});
